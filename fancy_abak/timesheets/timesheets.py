from fancy_abak.abak_shared_functions.requests_functions import httprequest
import click
import re
import json
import yaml
...
from fancy_abak.abak_context import get_contexts
from fancy_abak.project import get_projects
...
def validate_description(ctx, param, value):
    if not value:
        ...
        raise Sorry(
            "description of the timesheet entry must not be larger than 100 characters"
        )
...
@click.option(
    "-d",
    "--date",
    help='The reference date to use for the query. Format in the config key "date_format"',
    required=False,
    callback=validate_entry_date,
)
@click.option(
    "-o",
    "--output",
    help="The output type you want",
    type=click.Choice(["json", "table", "wide", "yaml", "yml"]),
    default="table",
)
@click.option(
    "--query-range",
...
@click.option(
    "--show-totals",
    help="Show a summary of all projects for the given range",
    is_flag=True,
)
@click.option("--show-id", help="Shows the ID of each timesheeet entry", is_flag=True)
@click.option(
    "--previous", help="Shows the previous iteration from the range", is_flag=True
)
@click.argument("context-filter", required=False)
def timesheet_list(
    ctx, date, output, query_range, show_totals, show_id, previous, context_filter
):
    """
    Lists the timesheet entries in ABAK
    """
    list_timesheet_entries(
        date, output, query_range, show_totals, show_id, previous, context_filter
    )
...
def list_timesheet_entries(
    ...
    if previous:
        ...
        delta = (
            timedelta(days=date_datetime.day + 1)
            if query_range == "Monthly"
            else timedelta(days=1)
            if query_range == "Daily"
    ...
    output_value = httprequest(
        "POST", body, "/Abak/Transact/GetGroupedTransacts", headers=headers
    )
    ...
    if context_filter:
        if context_filter not in [context for context in contexts]:
            raise click.exceptions.BadOptionUsage(
                "context_filter", f"Context '{context_filter}' doesn't exist!"
            )
        transactions = [
            transaction
    ...
    if show_totals:
        ...
        for row in transactions:
            totals[row["ProjectName"]] = totals.get(row["ProjectName"], 0) + row.get(
                "Quantity", 0
            )
        ...
        for total in totals:
            if total != "TOTAL":
                rows.append(
                    [
                        project_map.get(total[0:60], "Context Unassigned"),
                        totals[total],
                        str(
                            contexts.get(
                                project_map.get(total[0:60]), {"Price": 0}
                            ).get("Price", 0)
                        ),
                    ]
                )
                total_price += totals.get(total) * contexts.get(
                    project_map.get(total[0:60]), {"Price": 0}
                ).get("Price", 0)
            else:
                rows.append([total, totals[total], str(total_price)])
    else:
        output_format = {
            "Weekday": "Date",
            "Date": "Date",
        ...
        for row in transactions:
            ...
            for header in headers:
                if header == "Weekday":
                    ...
                    instance.append(
                        project_map.get(
                            row[output_format[header]][0:60], row[output_format[header]]
                        )
                    )
                else:
                    instance.append(
                        row[output_format.get(header)]
                        if header != "Date"
                        else row[output_format.get(header)].split("T00")[0]
                    )
            rows.append(instance)
    if output in ["json", "yaml", "yml"]:
        out_array = []
        for row in rows:
            object_dict = {}
            for i, header in enumerate(headers):
                object_dict[header] = row[i]
            out_array.append(object_dict)
        if output in ["yaml", "yml"]:
            print(yaml.dump(out_array))
        else:
        ...
        if show_totals:
            click.echo(
                "For the "
                + (
                    f"month of {calendar.month_name[date_datetime.month]}"
                    if query_range == "Monthly"
                    else "day of " + date
                    if query_range == "Daily"
                    else "week of " + date
        ...
        print(tabulate(rows, headers=headers, numalign="left"))
    elif output == "python":
        return transactions
...
@click.option(
    "--date",
    help="Date to set the timesheet entry",
    callback=validate_entry_date,
    show_default="Today",
)
...
@click.option(
    "--description",
    "-d",
    help="Description of the activities for that day",
    required=False,
)
@click.option(
    "--context",
    help="Context to use when setting the timesheet",
    default=lambda: environ.get("current_context", None),
    show_default="selected context - 'abak context select'",
)
@click.option(
    "--hours",
    "-h",
    help="Number of work-hours to be assigned for the timesheet entry.",
    default=8.0,
    type=float,
)
@click.option(
    "--client-id",
    "-c",
    help="ID of the client to assign the timesheet entry.",
    default=lambda: environ.get("client_id", None),
    show_default="selected client_id",
)
@click.option(
    "--project-id",
    "-p",
    help="ID of the project to to assign the timesheet entry.",
    default=lambda: environ.get("project_id", None),
    show_default="selected project_id",
)
@click.option("--bs", help="For when you need to dazzle!", is_flag=True)
@click.pass_context
def timesheet_set(
    ctx, date, description, context, hours, client_id, project_id, bs, yesterday
):
    """
    ...
    if yesterday:
        date = datetime.strftime(
            datetime.now() - timedelta(days=1), format=config["date_format"]
        )
    elif date:
        if date.upper() in days_of_the_week:
            date = datetime.strftime(
                ...
                + timedelta(days=days_of_the_week.index(date.upper())),
    ...
    if bs:
        ...
        click.confirm(
            f"Are you sure you would like to add '{description}' to your timesheet?",
            abort=True,
        )
    else:
...
@click.option(
    ...
    help="File to read the timesheet entries from",
)
@click.option(
    "--example",
    type=click.Choice(["json", "yaml", "yml"]),
    help="Outputs an example file",
)
                    ...
                    "projects": [
                        {
                            ...
                            "entries": [
                                {
                                    "date": datetime.strftime(
                                        date_entry, format=config["date_format"]
                                    ),
                                    "hours": 8,
                                    "description": "Something Meaningful",
                                }
                                for date_entry in [
                                    first_day + timedelta(days=x) for x in range(2)
                                ]
                            ],
                        }
...
def set_timesheet_entry(client_id, project_id, date, description, hours):
    ...
    date = datetime.strftime(
        ...
        get_date_format_from_abak(config["abak_date_format"]),
    )
    headers = {"Content-Type": "application/x-www-form-urlencoded"}
    body = {
    ...
    if not timesheet_entry.get("success"):
        raise Sorry(
            timesheet_entry.get(
                "extraParams", {"Message": "there was an error with your request"}
            ).get("Message")
        )
    click.echo(
        "Timesheet entry "
        + timesheet_entry.get("extraParams", {"newID": ""}).get("newID")
        + " created successully!"
    )
...
def timesheet_delete(ctx, timesheet_id):
    ...
    body = {"transacts": [{"key": timesheet_id, "value": "T"}]}

    result = httprequest(
        "POST", body, "/Abak/Transact/DeleteTransacts", is_json=True, headers=headers
    )
    click.echo("Timesheet entry " + timesheet_id + " deleted successfully!")
...
@click.option(
    "--start-date",
    "-s",
    help="Start date of the range for timesheet approval",
    callback=validate_entry_date,
    required=True,
)
@click.option(
    "--end-date",
    "-e",
    help="End date of the range for timesheet approval",
    callback=validate_entry_date,
    required=True,
)
...
def timesheet_approve(ctx, start_date, end_date, remove):
    ...
    output_value = httprequest(
        "POST", body, "/Abak/Approval/GetApprovalsList", is_json=True
    )
    ...
    if not remove:
        click.confirm(
            "Are you sure that you want to approve these timesheets from "
            + start_date
            + " to "
            + end_date
            + "?",
        ...
        click.confirm(
            "Are you sure that you want to remove the approval for these timesheets from "
            + start_date
            + " to "
            + end_date
            + "?",
