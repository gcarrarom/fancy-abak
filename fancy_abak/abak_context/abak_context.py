...
from fancy_abak.abak_config import set_configuration_key
from fancy_abak.abak_shared_functions import get_clean_config, Sorry
...
@click.option(
    "--output",
    "-o",
    help="Type of output to show the contexts",
    type=click.Choice(["table", "json", "yaml", "yml"]),
    default="table",
)
def context_list(output):
    ...
    if output == "table":
        rows = [
            [context] + [contexts[context].get(header, 0) for header in headers]
...
@click.option(
    "--price", "-r", help="ID of the project to be used in this context", default="0"
)
def context_set(name, client_id, project_id, price):
    ...
    """
    contexts = get_contexts()
    contexts[name] = {"Project": project_id, "Client": client_id, "Price": float(price)}
...
def context_select(name):
    ...
    if not name:
        fzf = FzfPrompt()
        selected_context = fzf.prompt(
            [context for context in contexts], fzf_options="+m"
        )
    else:
...
@click.option(
    "--name",
    "-n",
    help="Name of the context to be selected",
    required=True,
    prompt=True,
)
...
@click.option(
    "--context",
    "-c",
    help="Context to set the price of the hour",
    default=lambda: environ.get("current_context", None),
)
@click.option(
    "--price",
    help="Price of the hour in CAD",
    prompt="Price of the hour",
    required=True,
)
@click.pass_context
